...
if __name__ == '__main__':
    ...
    with open('freq_items_dataset.txt', 'r') as f:
        for line in f:
            transactions.append(line.strip().rstrip('\n').split(' '))
    midtime = timeit.default_timer()
    # print transactions
    print midtime - start
    result = []
    for itemset, support in find_frequent_itemsets(transactions, 100, True):
        result.append((itemset,support))

    endtime = timeit.default_timer()
    print endtime-midtime
    result = sorted(result, key=lambda i: i[0])
    outputFile = open('fp_output.txt', 'w')
    for itemset, support in result:
        outputFile.write(str(itemset) + ' ' + str(support) + '\n')
        print(str(itemset) + ' ' + str(support))
    outputFile.write(str(start) + '\n')
    outputFile.write(str(midtime) + '\n')
    outputFile.write(str(endtime) + '\n')
    outputFile.close()