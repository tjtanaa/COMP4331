	...
	for ptIndex in neighborPtsIndex:
		...
		if(ptStatus[ptIndex] == 0):
			...
			if(len(pt_neighborPtsIndex) >= minPts):
				...
				tempNeighbor = [pt_i for pt_i in pt_neighborPtsIndex if(not(pt_i in neighborPtsIndex))]
				############### EQUIVALENT to the code below ###########
				# tempNeighbor = []
				# for pt_i in pt_neighborPtsIndex:
				# 	if pt_i in neighborPtsIndex:
				# 		continue
				# 	else:
				# 		tempNeighbor.append(pt_i)
				# 		ptLabels[pt_i] = C
				####################################################
...
def _user_DBSCAN(data, eps = 0.12, minPts = 3):
	...
	for pindex, p in enumerate(data):
		if(ptStatus[pindex] == 0): # unvisited points
			ptStatus[pindex] = 1 # marked as visited
			# return all points within p's eps-neighborhood(including p)
			# return indexes of points within p's eps-neighborhood(including p)
			# print "DBcheckPoint1"
			neighborPtsIndex = regionQuery(p, eps, data)
			# print "DBcheckPoint2"
			if(len(neighborPtsIndex) < minPts):
				ptLabels[pindex] = -1 # NOISE == -1
			else:
				C += 1 # next cluster
